...
import { ChevronUp, X, Hand } from "lucide-react";
...
const DynamicScrolland = ({
  ...
  const prog = <Progress sp={sp} scrollDirection={scrollDirection} {...p} />;
  const items = <Items {...p} />;
    ...
    <MotionConfig transition={transition}>
      <AnimatePresence mode="wait" initial={false}>
        {open && (
          <motion.div
            ...
            className="bg-black/10 fixed inset-0 z-50 backdrop-blur-[4px]"
      ...
      >
        <motion.div
          ...
          className={cn(
            "relative flex h-10 cursor-pointer items-center overflow-hidden px-1 outline-none",
        ...
        >
          ...
          <div className="absolute top-0 right-0 bottom-0">{prog}</div>
        </motion.div>
...
export default DynamicScrolland;
...
function Progress({
  ...
  lPrefix,
  scrollDirection,
}: Props & { sp: MotionValue; scrollDirection: "up" | "down" | "idle" }) {
  ...
  // Calculate hand position based on scroll direction
  ...
  // Get hand rotation based on scroll direction
...
function Items({ setValue, data }: Props) {
  return (
    <div className="group grid gap-1 transition-opacity">
      {data.map((i) => (
        <button
          key={i.name}
          onClick={() => setValue?.(i)}
          aria-label={i.name}
          className="cursor-pointer text-left font-semibold transition-all group-hover:opacity-40 hover:!opacity-100 py-1 px-2 rounded hover:bg-white/10"
        >
          {i.name}
        </button>
      ))}
